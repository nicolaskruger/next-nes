...
const isINDEXED_INDIRECT = (addr: string) =>
  /^\(\$[0-9A-F]{2},X\)$/g.test(addr);
...
const toOpcode =
  (regex: RegExp, parse: (value: string) => number[]) => (value: string) => {
...
export const relativeToOpcode = toOpcode(/^\*([+-]\d{1,3})$/g, (value) => [
  Number(value) & 0xff,
]);

const toOpcodeTwoBytes = (regex: RegExp) =>
  toOpcode(regex, parseHexTwoBytesToArray);
...
export const indexedIndirectToOpcode = toOpcodeOneByte(
  /^\(\$([0-9A-F]{2}),X\)$/g
);

export const indirectIndexedToOpcode = toOpcodeOneByte(
  /^\(\$([0-9A-F]{2})\),Y$/g
);

const defaultToOpcode = (value: string) => [];

const addrDataDictionary: Dictionary<ADDR, (value: string) => number[]> = {
