...
const initBusRunner = (): Bus => "_".repeat(0x10000).split("").map(_ => ({
    data: 0,
    read: simpleRead,
    write: simpleWrite
}))
...
const run = async (program: string): Promise<Nes> => {
    ...
    if (comp.length > 32768)
        throw new Error("to long program max 32768 bytes")
    ...
    while (getPC(nes) < (0x8000 + comp.length)) {
        ...
        nes = instruction({ baseCycles, ...addrResult, offsetOnCross: offsetCycles })
    }
    return nes
}
